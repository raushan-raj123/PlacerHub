...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        $studentIdNew = sanitizeInput($_POST['student_id'] ?? '');
        ...
        // Validation
        if (empty($studentIdNew) || empty($name) || empty($email) || empty($course) || empty($department) || empty($admissionDate)) {
            ...
            try {
                ...
                $checkStmt = $db->prepare("SELECT id FROM students WHERE (student_id = ? OR email = ?) AND id != ?");
                $checkStmt->execute([$studentIdNew, $email, $studentId]);
                if ($checkStmt->fetch()) {
                    ...
                    $stmt = $db->prepare("
                        UPDATE students 
                        SET student_id = ?, name = ?, email = ?, phone = ?, course = ?, department = ?, 
                            year_of_study = ?, admission_date = ?, address = ?, status = ?, updated_at = NOW()
                        WHERE id = ?
                    ");
                    
                    $stmt->execute([
                        $studentIdNew, $name, $email, $phone, $course, $department, 
                        $yearOfStudy, $admissionDate, $address, $status, $studentId
                    ]);
                    ...
                    $stmt->execute([
                        $user['id'],
                        'Student Updated',
                        "Student '{$name}' has been successfully updated.",
                        'success'
                    ]);
        ...
        <header class="bg-white shadow-sm border-b border-gray-200">
            <div class="flex items-center justify-between px-6 py-4">
                <div class="flex items-center">
                    ...
                    <h1 class="ml-4 text-2xl font-semibold text-gray-800">Edit Student</h1>
                </div>
                
                <div class="flex items-center space-x-4">
                    <a href="view.php?id=<?php echo $student['id']; ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition duration-200">
        ...
        <main class="p-6">
            ...
            <nav class="mb-6">
                <ol class="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <li><i class="fas fa-chevron-right"></i></li>
                    <li><a href="view.php?id=<?php echo $student['id']; ?>" class="hover:text-gray-700"><?php echo htmlspecialchars($student['name']); ?></a></li>
                    <li><i class="fas fa-chevron-right"></i></li>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <?php if ($success): ?>
                    <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-6">
                        ...
                        <div class="mt-2">
                            <a href="view.php?id=<?php echo $student['id']; ?>" class="text-green-800 underline">View student details</a> |
                            <a href="index.php" class="text-green-800 underline">Back to students list</a>
                        </div>
                    </div>
                <?php endif; ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="course" name="course" required onchange="updateDepartment()"
                                    ...
                                    <option value="<?php echo htmlspecialchars($course['course_code']); ?>" 
                                            ...
                                            <?php echo ($_POST['course'] ?? $student['course']) === $course['course_code'] ? 'selected' : ''; ?>>
                            ...
                            <div>
                                ...
                                <select id="department" name="department" required
                                    ...
                                    <option value="<?php echo htmlspecialchars($department['dept_name']); ?>"
                                            <?php echo ($_POST['department'] ?? $student['department']) === $department['dept_name'] ? 'selected' : ''; ?>>
                            ...
                            <div>
                                ...
                                <input type="date" id="admission_date" name="admission_date" required
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                       value="<?php echo htmlspecialchars($_POST['admission_date'] ?? $student['admission_date']); ?>">
                            </div>
                            ...
                            <div>
                                ...
                                <select id="status" name="status"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                                    <option value="active" <?php echo ($_POST['status'] ?? $student['status']) === 'active' ? 'selected' : ''; ?>>Active</option>
                                    <option value="graduated" <?php echo ($_POST['status'] ?? $student['status']) === 'graduated' ? 'selected' : ''; ?>>Graduated</option>
                                    <option value="dropped" <?php echo ($_POST['status'] ?? $student['status']) === 'dropped' ? 'selected' : ''; ?>>Dropped</option>
                                    <option value="suspended" <?php echo ($_POST['status'] ?? $student['status']) === 'suspended' ? 'selected' : ''; ?>>Suspended</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <textarea id="address" name="address" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                      placeholder="Enter address"><?php echo htmlspecialchars($_POST['address'] ?? $student['address']); ?></textarea>
                        </div>
                    </div>

                    <!-- Form Actions -->
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                        <a href="view.php?id=<?php echo $student['id']; ?>" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition duration-200">
                            Cancel
                        </a>
